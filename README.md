...
[![Status](https://img.shields.io/badge/status-demo-blue)](https://github.com/Nikila99gimhan) [![AWS](https://img.shields.io/badge/AWS-Well--Architected-orange)](https://aws.amazon.com/architecture/well-architected/) [![CI/CD](https://img.shields.io/badge/CI%2FCD-AWS%20Native-green)](https://aws.amazon.com/devops/continuous-delivery/)
...
| `demo-on.sh` | Script to provision/resume expensive resources |
| `demo-off.sh` | Script to pause/delete expensive resources |
...
```mermaid
graph LR
    subgraph GitHub_Phase [GitHub Validation]
        GH[GitHub] --> PR[PR Security Gate]
        PR --> GL[Gitleaks]
        PR --> TR[Trivy FS]
        PR --> CO[CODEOWNERS]
    end
    ...
    subgraph AWS_Pipeline [AWS CodePipeline]
        CC[CodeConnections] --> CP[CodePipeline]
        CP --> CB[CodeBuild]
        CB --> ECR[ECR]
        CB --> Scan[Trivy Image Scan]
        Scan --> Deploy[ECS Fargate]
    end

    GH --> CC
    Deploy --> End[Live Endpoint]
```
...
| Set this up yourself | [Getting Started](#-getting-started) |
...
- **GitHub Account**: To host your fork.
- **AWS CLI**: Installed and configured with `AdministratorAccess`.
- **Node.js & Docker**: For local testing and initial builds.

### 1. Initial Setup (The One-Time Bridge)

1. **Fork this repository** to your personal GitHub account.
2. **Create the Connection**: Go to the AWS Console → **CodeConnections** → **Create connection**.
   - Select **GitHub**.
   - Follow the OAuth flow to install the AWS Connector for GitHub on your fork.
   - **Important:** Keep the Connection ARN; you will need it for the deployment scripts.

### 2. Provision Infrastructure

Run the "ON" script to create the VPC, ECS Cluster, and Pipeline.
```bash
chmod +x demo-on.sh
./demo-on.sh --connection-arn "arn:aws:codeconnections:..."
```

### 3. Trigger the First Deploy

1. Make a small change (e.g., edit the background color in `src/app.css`).
2. Commit and push:
   ```bash
   git add .
   ...
   ```
3. Open **CodePipeline** in the AWS Console to watch the six security gates in action.

### 4. Verify

Once the "Deploy" stage is green, find your **ALB DNS Name** in the ECS Service description or EC2 Load Balancers console. Visit the URL to see your live container.
...
graph TD
    subgraph VPC ["VPC (10.0.0.0/16)"]
        subgraph AZ_A ["AZ-A (ap-southeast-1a)"]
        ...
        subgraph AZ_B ["AZ-B (ap-southeast-1b)"]
        ...
        VPCE["VPC Endpoints<br/>(ECR, Logs, S3)"]
    end

    Internet((Internet)) -->|port 80| ALB_A
    Internet -->|port 80| ALB_B
    ALB_A --> Task_A
    ALB_B --> Task_B
    Task_A --> VPCE
    Task_B --> VPCE
...
![CI/CD Pipeline Architecture](./cicd-pipeline-diagram.png)
